...
@section('pageHeader')
    <x-page-header pageHeaderTitle="{{ __('td-cost-calcultaror::messages.edit_cost_item') }}" />
@endsection

@section('content')
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">{{ __('td-cost-calcultaror::messages.edit_cost_item') }}</h6>
        </div>
        <div class="card-body">
            <form action="{{ route('td-cost-calcultaror.cost-items.update', $costItem->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" 
                           value="{{ old('name', $costItem->name) }}" required>
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" class="form-control @error('price') is-invalid @enderror" id="price" 
                               name="price" value="{{ old('price', $costItem->price) }}" step="0.01" min="0" required>
                        @error('price')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('period') is-invalid @enderror" id="period" name="period" required>
                        <option value="month" @if(old('period', $costItem->period) == 'month') selected @endif>
                            {{ __('td-cost-calcultaror::messages.period_month') }}
                        </option>
                        <option value="year" @if(old('period', $costItem->period) == 'year') selected @endif>
                            {{ __('td-cost-calcultaror::messages.period_year') }}
                        </option>
                        <option value="hour" @if(old('period', $costItem->period) == 'hour') selected @endif>
                            {{ __('td-cost-calcultaror::messages.period_hour') }}
                        </option>
                        <option value="minute" @if(old('period', $costItem->period) == 'minute') selected @endif>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @error('category_id') is-invalid @enderror" id="category_id" name="category_id">
                            ...
                            @foreach($categories as $id => $name)
                                <option value="{{ $id }}" @if(old('category_id', $costItem->category_id) == $id) selected @endif>{{ $name }}</option>
                            @endforeach
                        </select>
                        @error('category_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="metadata_lifetime" name="metadata[lifetime]" 
                                   value="{{ old('metadata.lifetime', $costItem->getMetaField('lifetime')) }}" min="0">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="metadata_capacity" name="metadata[capacity]" 
                                   value="{{ old('metadata.capacity', $costItem->getMetaField('capacity')) }}" min="0">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="metadata_notes" name="metadata[notes]" rows="3">{{ old('metadata.notes', $costItem->getMetaField('notes')) }}</textarea>
                        </div>
                    </div>
                </div>
